...
\begin{equation}
	k_X = f(t) + \alpha k^L + \tilde\alpha k^R \label{y20potentialansatz}
\end{equation}
...
Having ascertained we would like $\hatt v \propto \sigma$ and $\tildd v \propto \delta$, one would like to also fix the right normalization for the K\"ahler moduli. We now consider the two homology 2-cycles $C^1$, $C^2$ given by the two basal $\mathbb{CP}^1$ respectively. We note that
...
\begin{align}
	\hatt\omega = \omega_1 = \frac{1}{2\pi}\left( \pder{J}{\sigma} \right) = \pder{J}{\hatt v} && \hatt v := 2\pi \sigma
	\label{}
\end{align}

which makes it so $\int_{C^i} \omega = 2$ is integer. The dual to $\hatt \omega $ is $-2(D^1+D^2) =: E$; it is easy to show this is actually the base $\mathbb{CP}^1 \times \mathbb{CP}^1$.
...
\begin{align}
	I_0 = \int \hatt\omega \wedge \hatt\omega \wedge \hatt\omega = E \cdot E \cdot E \\
	I_1 = \int \hatt\omega \wedge \hatt\omega \wedge \tildd\omega = E \cdot E \cdot F \\
	I_2 = \int \hatt\omega \wedge \tildd\omega \wedge \tildd\omega = E \cdot F \cdot F
	\label{}
\end{align}
...
\begin{center}\begin{tabular}{c | l c}
	...
	$\hatt\rho = \rho_1$ & dual to $\hatt v$ & 4-cycle blowup deformation of $X$\\
	$\tildd\rho = \rho_2$ & dual to $\tildd v$ & 2-cycle blowup deformation of $X$\\
...
We can match these directly with the field theory objects discovered in section \ref{sec:squares}. $z_I^i$ map directly with the $3N$ mesons parametrizing $\mmes$. $\hatt\rho$, $\tildd\rho$ match with the two baryons generating the resolution of the cone as in \ref{sec:squaresmoduli}, while $\beta$ is the third ``non-geometric'' baryonic modulus.

The chiral fields $\rho_a = (\hatt \rho,\tildd{\rho})$ are related to the moduli $v_a = (\hatt v, \tildd v)$ by the Legendre transform described in the previous chapter; as anticipated we will only need to specialize the precise form of the real part of $\rho_a(v_a)$:
...
where $\kappa_a(z_I,\bar z_I; v) = (\hatt\kappa,\tildd\kappa)$ are defined as the potentials that generate the $\omega_a$, as in
...
\begin{align}
 	2\pi \hatt\kappa(t;\sigma,\delta) =	\pder{k_X}{\sigma} & = \left( \int d\, \ln t' \pder{y}{\sigma} \right) + \frac{1}2 k^L + \frac{1}2 k^R \\
	2\pi \tildd\kappa(t;\sigma,\delta) =  \pder{k_X}{\delta} & = \left( \int d\, \ln t' \pder{y}{\delta} \right) + \frac{1}2 k^L - \frac{1}2 k^R
	\label{}
\end{align}
